...
import com.KayraAtalay.dto.DtoOrderItem;
...
public class DtoConverter {
	...
	public static DtoOrderItem toDto(OrderItem orderItem) {
		DtoOrderItem dtoOrderItem = new DtoOrderItem();
		BeanUtils.copyProperties(orderItem, dtoOrderItem);

		dtoOrderItem.setProduct(toDto(orderItem.getProduct()));
	...
	public static DtoOrder toDto(Order order) {
		...
		List<DtoOrderItem> items = order.getItems().stream().map(DtoConverter::toDto).collect(Collectors.toList());

		BeanUtils.copyProperties(order, dtoOrder);
		dtoOrder.setCustomer(toDto(order.getCustomer()));
		dtoOrder.setDeliveryAddress(toDto(order.getDeliveryAddress()));
		dtoOrder.setItems(items);
